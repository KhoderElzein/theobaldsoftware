...
{    
     ...
     try
    {
        ...
        # On SP Server 
        #$con = New-Object ERPConnectServices.ERPConnectServiceClient($SapConnection) 

        # On machine without SP 
        $con = New-Object ERPConnectServices.ERPConnectServiceClient($WebURL, $SapConnection) 
        <#
        if([string]::IsNullOrEmpty($SapConnection)){
            $con = New-Object ERPConnectServices.ERPConnectServiceClient($WebURL) 
        }
        else {
            $con = New-Object ERPConnectServices.ERPConnectServiceClient($WebURL, $SapConnection) 
            #$con = New-Object ERPConnectServices.ERPConnectServiceClient()
        }
        ...
        if ($con -ne $null){
            ...
            return ,$con.ExecuteTableQuery($TableName, $tableSettings) 

        } else {
            write-host "SAP Connection for Site URL '$WebURL' and Sytem '$SapConnection' can't be created!" -ForegroundColor Red
        }                      
    }
    catch{
            $ErrorMessage = $_.Exception.Message
...
{    
    ...
    $Table = GetSAPTable -SapConnection $SapConnection -WebURL $WebURL -TableName $TableName -RowCount $RowCount -Fields $Fields -WhereClause $WhereClause -CustomFunction $CustomFunction 
    Write-TableToList -Credentials $Credentials -Table $Table -ListName $ListName -WebURL $WebURL -ListMode $ListMode
...
{    
     ...
     try
    {
        ...
        # Truncate
        # Drop (default) 
        [System.Data.DataTable] $Table = GetSAPTable -sapConnection $SapConnection -tableName $TableName -fields $Fields -whereClause $WhereClause -rowCount $RowCount -customFunction $CustomFunction -WebURL $WebURL
